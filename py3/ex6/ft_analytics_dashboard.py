if __name__ == "__main__":
    try:
        ...
        players = [
            {"name": "alice", "region": "north", "achievements":
             "boss_slayer"},
            {"name": "bob", "region": "east", "achievements": "level_10"},
            {"name": "charlie", "region": "central", "achievements":
             "first_kill"},
        ...
        print(f"Average score: "
              f"{sum(scores_doubles) / 2 / len(uni_players):.1f}")
        print(f"Top performer: {top_play} ({more_score} "
              f"points, {top_achieve} achievements)")

    except KeyError as e:
        print(f"{e.__class__.__name__}: The keys is wrong, expeted {e}")
